...
const reloadRealUsers = async () => {
  ...
  if (!isLocalMode && (!role || role === 'checking')) return <div className="bg-gray-800 p-4 rounded border border-gray-700">Admin role required or role API failed.
...
UID: {authUserId || '(unknown)'}</div>;
  return <div className="space-y-6 animate-fade-in">
    ...
    <section className="travel-card gs-admin-panel p-4">
    ...
    <section className="travel-card gs-admin-panel p-4">
      ...
      <div className="mb-4 p-3 bg-gray-900 rounded border border-gray-700">
        ...
        <div className="space-y-2">
          ...
          <input className="travel-input w-full bg-gray-800 border border-gray-600 rounded p-2 text-xs" placeholder="Temporary password / new password" type="password" value={addUserPassword} onChange={(e) => setAddUserPassword(e.target.value)} />
          <div className="flex gap-2">
            ...
            <button className="travel-btn travel-btn--success px-3 py-2 rounded text-xs" onClick={() => guarded(async () => { if (!addUserEmail.trim()) { setMessage('Email required'); return; }
            ...
            <div key={user.user_id} className="bg-gray-900 p-4 rounded border border-slate-700">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  <p className="text-slate-100 font-semibold text-sm">{user.display_name || user.email || 'User'}</p>
                  <p className="text-slate-400 text-xs break-all">{user.email || 'EMAIL_MISSING_FROM_API'}</p>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex gap-2">
                    <select
                      className="travel-input text-xs bg-gray-800 border border-gray-600 rounded px-2 py-2 w-full"
                    ...
                    <button
                      className="travel-btn travel-btn--primary px-3 py-2 rounded text-xs"
                      type="button"
                  ...
                  <div className="flex gap-2">
                    ...
                    <button className="travel-btn travel-btn--danger px-3 py-2 rounded text-xs flex-1" onClick={() => guarded(async () => { if (confirm('Delete user?')) { setBusyUserId(user.user_id);
