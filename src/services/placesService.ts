...
import { listLocalStagingPlaces } from './localStorageProvider';

export type PlaceStatus = 'pending' | 'approved' | 'rejected';
export type PlaceRecord = {
  id?: string; source_staging_id?: string | null; name: string; province: string; category: string; other_category?: string | null; sub_category?: string | null; google_maps_url?: string | null; phone?: string | null; recommender?: string | null; suggestion?: string | null; rejection_reason?: string | null; rating: number; amenities: string[]; alerts: string[]; status?: PlaceStatus; created_at?: string; updated_at?: string;
};
...
export async function saveStagingPlace(place: PlaceRecord) {
  ...
  try {
    const payload = { ...place, status: place.status || 'pending', updated_at: new Date().toISOString() };
